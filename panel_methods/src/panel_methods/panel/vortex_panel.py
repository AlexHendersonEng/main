...
class VortexPanelSystem:
    """
    Assembles and solves a vortex panel method for a closed body in a uniform flow.
    """
    ...
    def solve(self):
        """
        Solves for panel vortex strengths and computes surface velocities and
        ...
        for i in range(self.n_panels):
            ...
            for j in range(self.n_panels):
                ...
                a3n = -np.cos(phi_i - phi_j)
                a4n = (x_ic - x_j1) * np.cos(phi_i) + (y_ic - y_j1) * np.sin(phi_i)
                an[i, j] = -integration_term(a1, a2, a3n, a4n, a5, s_j)

                # Tangential influence coefficient
                a3t = np.sin(phi_j - phi_i)
                a4t = (x_ic - x_j1) * np.sin(phi_i) - (y_ic - y_j1) * np.cos(phi_i)
