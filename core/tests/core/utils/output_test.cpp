...
TEST_F(ToCsvTest, WritesCorrectHeader) {
  ...
  ASSERT_EQ(core::utils::ToCSV(test_file_, t, y), 0);
...
TEST_F(ToCsvTest, ReturnsZeroOnSuccess) {
  ...
  EXPECT_EQ(core::utils::ToCSV(test_file_, t, y), 0);
}
...
TEST_F(ToCsvTest, FileCreatedOnDisk) {
  ...
  core::utils::ToCSV(test_file_, t, y);
...
TEST_F(ToCsvTest, ReturnsOneForInvalidPath) {
  ...
  EXPECT_EQ(core::utils::ToCSV("/nonexistent_dir/output.csv", t, y), 1);
}
