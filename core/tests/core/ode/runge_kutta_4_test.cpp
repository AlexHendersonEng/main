...
TEST_F(RungeKutta4Test, NegativeStepSize) {
  ...
  EXPECT_THROW(
      { core::ode::RungeKutta4(f, 0.0, 1.0, {1.0}, -0.1); },
...
TEST_F(RungeKutta4Test, ZeroStepSize) {
  ...
  EXPECT_THROW(
      { core::ode::RungeKutta4(f, 0.0, 1.0, {1.0}, 0.0); },
      std::invalid_argument);
}
...
TEST_F(RungeKutta4Test, InvalidTimeInterval) {
  ...
  EXPECT_THROW(
      { core::ode::RungeKutta4(f, 1.0, 0.0, {1.0}, 0.1); },
      std::invalid_argument);
}
...
TEST_F(RungeKutta4Test, EqualStartEndTime) {
  ...
  EXPECT_THROW(
      { core::ode::RungeKutta4(f, 1.0, 1.0, {1.0}, 0.1); },
      std::invalid_argument);
}
...
TEST_F(RungeKutta4Test, ConstantFunction) {
  ...
  core::ode::Solution sol = core::ode::RungeKutta4(f, 0.0, 1.0, {5.0}, 0.1);
...
TEST_F(RungeKutta4Test, LinearFunction) {
  ...
  double t1 = 1.0;
  double h = 0.1;
  core::ode::Solution sol = core::ode::RungeKutta4(f, t0, t1, {0.0}, h);
...
TEST_F(RungeKutta4Test, ExponentialGrowth) {
  ...
  double t1 = 1.0;
  double h = 0.01;
  core::ode::Solution sol = core::ode::RungeKutta4(f, t0, t1, {1.0}, h);
...
TEST_F(RungeKutta4Test, MultiDimensionalSystem) {
  ...
  core::ode::Solution sol = core::ode::RungeKutta4(f, t0, t1, y0, h);
