...
MODELICA_SOURCE = """model MassSpringDamper
  constant Real m = 1;
  parameter Real k = 2;
  parameter Real c = 0.2;
  Real position(start=1, fixed=true);
  Real velocity(start=0, fixed=true);
equation
  der(position) = velocity;
  der(velocity) = -(k*position + c*velocity)/m;
...
def build_step_integrator(dt: float) -> ca.Function:
    """Build a one-step CasADi integrator from the Modelica mass-spring-damper model."""
    session = Session()
    model = session.loads(
        MODELICA_SOURCE,
        model="MassSpringDamper",
        filename="MassSpringDamper.mo",
    )
    casadi_model = model.to_casadi()
    return casadi_model.integrator(
        dt,
        method="cvodes",
        abstol=1e-10,
        reltol=1e-10,
